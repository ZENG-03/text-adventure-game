...
scenes["title"] = {
    ...
    on_enter: () => {
        ...
        if(globalState.endingsReached.length > 0) {
            msg = `<div class="system-message" style="color:var(--hover-color);">【轮回记录】<br>已轮回次数：${globalState.playCount}<br>已解锁结局：${globalState.endingsReached.length}/4<br>（包含：${globalState.endingsReached.join('，')}）</div>`;
        }
        return msg;
    },
    options: [
...
**获得线索：需要先调和四元素产生火焰，再将金属材料放入坩埚炼制**`,
    options: [
...
**获得：七色花琥珀**
**获得线索：需要七种植物部位（根、茎、叶、花、果、种、苗）来激活七色花种**`,
    options: [
...
**获得线索：音叉与乐器的对应关系（音叉1→小提琴，音叉2→中提琴，等等）**`,
    options: [
...
选项：
先探索底层工坊 [前往 clocktower_workshop]
前往中层观察窗 [前往 clocktower_midfloor]
直接上顶层检查钟盘和操作台 [前往 clocktower_top]
检查楼梯间的墙壁是否有隐藏 [前往 clocktower_staircase]
分支：底层工坊`,
    options: [
...
选项：
阅读维修手册 [前往 clocktower_manual]
检查工作台上的零件 [前往 clocktower_parts]
打开铁皮柜 [前往 clocktower_cabinet]`,
    options: [
...
获得线索：配电箱位置
选项：
继续探索工坊 [返回 clocktower_workshop]
去寻找配电箱 [前往 clocktower_powerbox]`,
    options: [
...
选项：
检查观测记录 [前往 clocktower_observations]
研究星图 [前往 clocktower_starchart]
检查窗户边缘是否有机关 [前往 clocktower_window_mechanism]`,
    options: [
...
获得线索：窗户刻度盘
选项：
记录下当前刻度 [前往 clocktower_record_scales]
继续探索 [返回 clocktower_midfloor]`,
    options: [
...
选项：
检查操作台 [前往 clocktower_control_panel]
查看钟盘背后的齿轮组 [前往 clocktower_gears]
检查墙上的布告板 [前往 clocktower_bulletin]
使用维修手册笔记（若有） [前往 clocktower_follow_manual]`,
    options: [
...
选项：
尝试转动圆盘 [前往 clocktower_turn_disk]
先寻找电源开关 [前往 clocktower_powerbox]`,
    options: [
...
获得：克劳利天文笔记
选项：
阅读笔记 [前往 clocktower_read_astro_notes]
继续寻找电源开关 [前往 clocktower_powerbox]`,
    options: [
...
状态：钟楼断电
选项：
返回顶层，尝试拨动指针 [前往 clocktower_adjust_safe]
先更换擒纵轮（若有） [前往 clocktower_replace_escapement]`,
    options: [
...
状态：擒纵轮已更换
选项：
继续拨动指针 [前往 clocktower_adjust_safe]`,
    options: [
...
选项：
先检查布告板 [前往 clocktower_bulletin]
直接拨到12点 [前往 clocktower_set_12]
根据已有线索拨到月升时刻（需要具体数值） [前往 clocktower_set_moonrise]`,
    options: [
...
选项：
将分针拨到47分位置 [前往 clocktower_set_moonrise_detailed]
将时针拨到9-10之间 [前往 clocktower_set_moonrise_detailed]`,
    options: [
...
选项：
尝试直接拨动指针（未断电） [前往 clocktower_electrocution]
尝试拉动拉杆（未断电） [前往 clocktower_trap_lever]`,
    options: [
...
返回大厅 [前往 hall_injured]
其他解谜路径：使用月相计算
如果玩家没有找到维修手册，但通过观测记录和天文笔记，仍然可以正确调整时间。我们可以设计一个简单的月相谜题：根据观测记录中7月15日的月升时间21:47，结合当前日期（假设游戏时间是7月15日午夜），玩家需要将钟表调到21:47。但玩家需要知道当前时间，这可以从钟楼内一个停止的摆钟或日历获得。我们可以在工坊或布告板上放一个停摆的钟，显示21:47，作为提示。`,
    options: [
...
选项：
将钟楼时间调至座钟时间 [前往 clocktower_set_moonrise_detailed]（若已断电）
隐藏区域与额外道具`,
    options: [
...
**获得：红宝石徽章（2/7）**
**获得：便条（内容：“齿轮钥匙可开启更多秘密。VII号门后，有通往画室的密道。”）**`,
    options: [
        { text: "返回大厅", target: "hall" },
...
**获得：紫藤花种子（可种在庄园花园）**
**获得：埃莉诺的祝福（在最终密室中，你将获得一次“灵感”加成，自动破解一个难题）**
...
管家曾提醒过你，强行拨动指针可能触发机关。`,
    options: [
        ...
        { text: "翻阅克劳利的日记（若有）", target: "clocktower_diary_solution", condition: () => hasItem("生命之露") },
...
墙上的乐谱架上放着一首未完成的乐曲，标题是《七重奏鸣曲》。乐谱最后一行写着：“唯有共鸣之心，可奏响遗失之章。”`,
    options: [
        ...
        { text: "使用机械齿轮（若有）", target: "musicroom_gear_solution", condition: () => hasItem("生命之露") },
        { text: "--", target: "hall_main" }
    ]
};
...
墙上挂着一幅未完成的肖像画，画中人物手中握着一面镜子，镜中倒映着七种颜色，但顺序混乱。`,
    options: [
        ...
        { text: "使用共鸣水晶（若有）", target: "studio_crystal_solution", condition: () => hasItem("生命之露") },
        { text: "--", target: "hall_main" }
    ]
};
...
树下有一个石盆，盆中盛着浑浊的液体。周围有七个花盆，分别标着七种颜色，但全部空空如也。`,
    options: [
        ...
        { text: "使用神秘颜料（若有）", target: "greenhouse_pigment_solution", condition: () => hasItem("生命之露") },
        { text: "--", target: "hall_main" }
    ]
};
...
衣柜门半开，里面似乎藏着什么。`,
    options: [
        ...
        { text: "使用符文石（若有）", target: "bedroom_rune_solution", condition: () => hasItem("生命之露") },
        { text: "--", target: "hall_main" }
    ]
};
...
阿斯特·克劳利
1897年秋”
你将钥匙插入大门的锁孔，庄园的大门在你身后缓缓关闭。管家奥尔德斯默默站在大厅里，向你鞠躬：“主人，欢迎回家。”
从今以后，你成了谜语馆的新主人。你守护着七谜考验，等待下一个探索者的到来。日复一日，年复一年，你逐渐理解了阿斯特的孤独——拥有整个庄园，却无法拥有外面的世界。偶尔，你会站在窗前，看着远方，想起自己曾经拥有的自由。但门已经关上，钥匙在你手中，你却没有勇气打开它。
多年后，一个年轻的探索者敲响了谜语馆的大门。你看着他走进大厅，眼神中闪烁着与你当年一样的好奇。你知道，轮回开始了。
（结局：永恒的回廊）
你拿起阿斯特的谜题笔记，将钥匙放回匣子。管家为你打开庄园的大门，晨光洒在你脸上。你深吸一口气，第一次感到空气如此清新。
离开谜语馆后，你将阿斯特的谜题整理出版，书名为《谜语馆的七重考验》。它成了畅销书，无数人通过你的书体验了解谜的乐趣。你成了知名的谜语作家，但你的心中始终有一块地方属于那座庄园。
偶尔，你会收到管家的信：“庄园一切安好，等待下一个主人。” 你知道，谜语馆还在那里，沉默地等待着。
（结局：自由的智者）
你将符文石放回地下洞穴的石柱，用守护者符文封印了古老的能量。管家的眼神中带着欣慰：“克劳利家族世代守护这个秘密。现在，它交给你了。”
你选择留在谜语馆，但不是作为主人，而是作为看门人。你定期检查封印，维护庄园，偶尔接待一些解谜者。你失去了离开的自由，但获得了一种更深层的安宁——你知道，地下的力量永远不会伤害任何人。
多年后，你在洞穴入口种下一棵紫藤。春天，紫藤花开，花香弥漫整个洞穴。你坐在花下，听着远处传来的音乐室琴声，感到一种奇特的圆满。
（结局：永恒的守护者）
你将阿斯特的信、伊莲娜的日记、埃莉诺的乐谱、托马斯的笔记整理成书，书名为《谜语馆的回响》。它既是谜题集，也是传记，更是一个时代的见证。
谜语馆被改建为博物馆和音乐厅。每年夏天，这里都会举办“谜语音乐节”，来自世界各地的解谜爱好者和音乐家齐聚一堂。埃莉诺的第七交响曲被完整演奏，伊莲娜的画作被永久陈列，托马斯的矿石标本供地质学家研究。
管家的身影最后一次出现在庄园门口。他微笑着对你说：“哥哥会为你骄傲的。” 然后，他消失在晨光中，像他说的那样，“去看看外面的世界”。
你站在紫藤架下，看着游客们在画室前驻足，在音乐室里聆听，在地质笔记前沉思。你知道，他们的故事不会被遗忘。
（结局：回响）
这个结局是结局四的深化版。当你做出选择后，密室中出现了异象——七色光汇聚成两个人影，一男一女，手牵着手。男人是阿斯特，女人是伊莲娜（或埃莉诺，取决于你对两位女性的理解深度）。他们向你微笑，然后消散在光芒中。
...
（真结局：七重谜语的真相）
如果你在回答阿斯特的问题时表现出贪婪、自私或对谜语精神的误解，密室会触发机关，将你送出庄园。管家站在门口，面无表情地说：“你不适合这里。”
大门在你身后关闭。你站在迷雾中，手里什么都没有。谜语馆消失了，像是从未存在过。你甚至开始怀疑，这一切是不是一场梦。
（结局：被遗忘的探索者）
| 支线 | 完成后的特殊选项 | 影响的结局 |
|------|------------------|------------|
| 管家支线 | 获得阿斯特的怀表，在最终密室解锁“成为守护者”的完整选项 | 结局三深化 |
| 画室支线 | 获得伊莲娜的纪念徽章，解锁结局四、五中的“纪念伊莲娜”对话 | 结局四、五 |
| 地下支线 | 获得符文石和守护者知识，解锁结局三 | 结局三 |
| 音乐室支线 | 获得夜莺徽章和埃莉诺的祝福，解锁结局四、五中的“演奏交响曲”选项 | 结局四、五 |
以上最终谜题和结局设计将之前的七谜主线与四个支线的所有伏笔串联起来，形成了多种可能的结局，每种结局都有其独特的主题和情感基调。您可以根据需要调整细节，确保每个结局都有足够的分量和回味空间。`,
    options: [
...
scenes["prologue_character"] = {
    desc: `**
> 你叫塞拉斯·诺斯，三十四岁，是迷雾城最出色的私家侦探。你以破解冷案和谜团闻名，从不依靠运气，只相信逻辑与观察。随身携带的侦探笔记记录了你经手的每一个案件——从失窃的油画到消失的遗产，没有一桩难倒过你。此刻，空白页正等待着新的谜题。
>
...
scenes["hall_initial"] = {
    desc: `**
> 门厅内，水晶吊灯蒙着薄灰，墙壁上挂着七幅抽象画，每一幅都似乎在注视着你。一位身着燕尾服、面容古板的老人无声地出现在你面前，眼睛如同深不见底的黑曜石。
>
> “欢迎，探索者。我是管家奥尔德斯，主人的意志将由我传达。”他的声音沙哑而平静，“主人留下遗嘱：庄园内设有七道谜题，分别位于七个房间。每解开一道，你将获得一枚宝石徽章。集齐七枚徽章，便可开启中央密室，获取遗产。但请记住——谜题可能致命，而选择不可逆转。你确定要开始吗？”
>
>`,
...
scenes["hall_1_medal"] = {
    desc: `**
> 你将[颜色]徽章放在壁炉台上。蓝宝石在火光中闪烁，映出一小片星图。管家瞥了一眼，微微点头：“第一枚。还有六道门等待着你。”
>
...
scenes["hall_3_medals"] = {
    desc: `**
> 三枚徽章在壁炉台上并排闪烁，光芒交织成柔和的虹彩。大厅的光线似乎比之前更亮了，三幅抽象画已经焕发出清晰的色彩——图书馆的书籍、钟楼的齿轮、音乐室的音符，栩栩如生。
>
...
scenes["hall_6_medals"] = {
    desc: `**
> 六枚徽章的光芒照亮了整个房间，壁炉台几乎成了一座小小的灯塔。墙上的六幅抽象画已经变成了生动的场景——图书馆的静谧、钟楼的庄严、音乐室的辉煌、画室的色彩、温室的生机、地下室的神秘。只差最后一幅。
>
...
scenes["hall_7_medals"] = {
    desc: `**
> 七枚徽章齐聚的瞬间，大厅中央的石台发出轰鸣，缓缓下沉，露出一条螺旋石梯，石阶向下延伸，消失在淡蓝色的荧光中。七幅抽象画同时燃烧起来——不是真的燃烧，而是化作七色光，汇聚成一道光束，照亮了石梯的入口。
>
...
scenes["final_chamber_transition"] = {
    desc: `**
> 当第七枚徽章嵌入油画的瞬间，整个卧室都开始震动。油画中的七扇窗户同时亮起，光芒汇聚成一束，射向地面。地板裂开，露出一条螺旋石梯，石阶向下延伸，消失在淡蓝色的荧光中。
>
